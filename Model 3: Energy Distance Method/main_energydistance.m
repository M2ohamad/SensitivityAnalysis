...
x1_LFI = sortrows( cat( 2, LF, Y), 1 ) ;    % Sorting by ascending L values
L_tI = zeros(1,length(k));
for i=1:length(k)
    nIntervals = k(i) ;
    N = k(i)^2 ; 
    intervalSize = N / nIntervals ;
    L_tI(i) = calc_totalInfluence(x1_LFI(1:N,:), Y(1:N), nIntervals, intervalSize) ; 
end
tI = cat( 1, L_tI, F_tI)';
labels = {'L Total Influence', 'F Total Influence'} ;
plot_convergence(tI,k.^2,[],[],[],[],[],labels);
