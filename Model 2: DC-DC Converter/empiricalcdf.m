...
x = x(:)      ;
N = length(x) ;
x = sort(x)   ;  % x is A values sorted smallest to largest
F = [1:N]/N   ;  % F is the A values normalized to 1.
...
[x,iu] = unique(x,'last'); % sorts X without any repetitions, otherwise, index of last value is used
F      = F(iu);
N      = length(F) ;
